static void init_pushButton() // pc1 push button > turns on pd3 led
 {	  
          PORTC.DIR &= ~(PIN1_bm);	  PORTC.PIN1CTRL |= PORT_PULLUPEN_bm; // pc1 as push button to be debounced with timer B1 as one shot 10ms delay
...
// TCB_1 init ...keeping tcb0 free , since i am using it for millis
	 TCB1.CCMP = 0xFFFF;
